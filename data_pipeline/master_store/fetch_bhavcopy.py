    ...
    while d <= end:
        ...
        for fmt, url in candidates(d):
            ...
            if status == 200 and body[:2] == b"PK":
                ...
                break
        if not got:
            if last_status == 404 and (date.today() - d).days <= 3:
                n_err += 1   # not published yet (the nightly runs at 19:30; NSE can be late) — leave it for the next night
