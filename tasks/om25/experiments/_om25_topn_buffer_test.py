...
3 × 3 grid:
  Top-N: 20, 25 (current), 30
  Buffer: 10, 15 (current), 20

Top-N controls portfolio diversification; smaller N = higher concentration.
Buffer controls "stickiness" of holdings; larger buffer = lower turnover from
rank churn at entry rebalances.
"""
import sys
...
def run(*, universe_path, entry_signal_dates, top_n, exit_buffer, label):
...
for u_name, u_path in universes:
    for c_name, sig_dates in cadences:
        print(f"\n=== {u_name} — {c_name} ===")
        print(f"{'Top/Buf':<10} {'CAGR':>7} {'Max DD':>8} {'LongDD':>7} "
        ...
        for top_n in tops:
            for buf in buffers:
                ...
                m = run(universe_path=u_path, entry_signal_dates=sig_dates,
                        top_n=top_n, exit_buffer=buf,
                        label=f'{u_name} {c_name} {cfg}')
                if m is None:
                    continue
                rows.append((cfg, m))
                print(f"{cfg:<10} "
