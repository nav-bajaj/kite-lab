...
The min_obs parameter controls how strict the data-quantity filter is —
the minimum number of valid daily returns required in the 252-day lookback
window before a stock is eligible for ranking.

Variants: 150 / 180 / 200 / 220 / 240 / 252
                              ^^^ current
"""
import sys
...
def run(*, universe_path, entry_signal_dates, min_obs, label):
    ...
    args = {'returns_universe': returns_uni, 'min_obs': min_obs}
    res = run_strategy(
...
for u_name, u_path in universes:
    for c_name, sig_dates in cadences:
        print(f"\n=== {u_name} — {c_name} ===")
        print(f"{'min_obs':<10} {'CAGR':>7} {'Max DD':>8} {'LongDD':>7} "
        ...
        for mo in variants:
            m = run(universe_path=u_path, entry_signal_dates=sig_dates,
                    min_obs=mo, label=f'{u_name} {c_name} mo={mo}')
            if m is None:
                continue
            rows.append((mo, m))
            print(f"{mo:<10} "
