"""OM25 production-config backtest from 2016-01-01 + comprehensive HTML report.

Locked-in config:
  Universe:      NSE Nifty 250
  Cadence:       Bi-weekly entry, weekly exit checks
  Score:         Bull 50/50 UC+CR / Bear CR-only (regime tilt)
  Regime:        NIFTY 100 100-DMA + 3-day confirmation
  Lookback:      252d, min_obs 220
  Top-N:         25, exit_buffer 20
  Return filter: ON
  Stop:          20% from peak (weekly check)
...
def compute_stats(values: pd.Series, dates: pd.Series) -> dict:
    """Comprehensive risk + return metrics from an equity series."""
    ...
    # Tail ratio: ratio of 95%-ile to abs 5%-ile
...
def compute_vs_index(port_rets: pd.Series, idx_rets: pd.Series) -> dict:
    """Beta, alpha, IR, up/down capture vs an index — only on overlapping dates."""
    ...
    # Alpha annualized: portfolio annualized excess - beta * index annualized excess
    rf_d = 0.05 / 252
...
def main():
    ...
    print(f"[run] OM25 from {START_DATE.date()} ...", flush=True)
    res = run_strategy(
    ...
    # ======== Index series — keep separate, don't dropna together ========
    ...
    # Build per-index aligned series (NaN where no data) — each normalized to 100 at its own first valid date intersected with portfolio start
    norm_port = 100.0 * port_pv / port_pv.iloc[0]
    norm_idx = {}
    norm_idx_starting_at_port_start = {}
    for name, s in idx_series.items():
        aligned = s.reindex(port_dates).ffill()
        # If aligned has NaN at start (index started later than port), keep NaN there
        first_valid = aligned.first_valid_index()
        if first_valid is None:
            continue
        first_val = aligned.loc[first_valid]
        # Two normalizations:
        # (a) "Indexed to OM25 start" — only valid where index has data; normalised to 100 on its first valid day in the OM25 calendar
        norm_idx[name] = (100.0 * aligned / first_val)
        # (b) "If we'd invested at OM25 start" — same but only if index existed; just used for plotting
        norm_idx_starting_at_port_start[name] = norm_idx[name]
    ...
    ax.plot(port_dates, norm_port, label="OM25",
            color=INDEX_COLORS["OM25"], linewidth=2.4)
    for name, s in norm_idx.items():
        ax.plot(s.index, s.values, label=name,
                color=INDEX_COLORS.get(name, "#666"),
                linewidth=1.0, alpha=0.85)
    ax.set_yscale("log")
    ax.set_title(f"OM25 vs Indices (₹100 indexed at start, log scale)",
    ...
    ax.fill_between(port_dates, port_dd.values, 0,
                    color="#1976d2", alpha=0.20)
    ax.plot(port_dates, port_dd.values, label="OM25",
            color=INDEX_COLORS["OM25"], linewidth=1.8)
    for name, s in norm_idx.items():
    ...
    ax.plot(rs.index, rs.values, color="#1976d2", linewidth=1.8, label="OM25")
    ...
    ax.set_title("OM25 Monthly Returns (%)", fontsize=14, fontweight="bold")
    ...
    yr_table["OM25"] = (yr_om["ret"] * 100).round(2)
    yr_table["OM25 DD"] = (yr_om["dd"] * 100).round(1)
    for col in INDICES:
        if col in yr_idx_dict:
            yr_table[col] = (yr_idx_dict[col]["ret"] * 100).round(2)
            yr_table[f"vs {col}"] = (yr_table["OM25"] - yr_table[col]).round(2)

    # ======== Current holdings & last-10-day PnL ========
    print(f"[holdings & recent PnL]...", flush=True)
    # Reconstruct holdings from trades
    held = {}  # symbol -> {shares, cost_basis, entry_date, entry_price}
    for _, tr in trades.iterrows():
        ...
        if side == "BUY":
            ...
            held[sym]["shares"] += sh
            held[sym]["cost_basis"] += sh * price * (1 + 0.002)  # include slippage
        else:
            if sym in held:
                # Reduce shares; reduce cost basis pro-rata
    ...
    for sym, info in held.items():
        if info["shares"] <= 0:
            continue
        # Get latest close from price panel
        ...
        holdings_rows.append({
            ...
            "notional": notional,
            "pnl_pct": pnl,
            "entry_date": info["entry_date"].date(),
            "days_held": days,
        })
    ...
    def fmt_inr(v):
        if v is None or pd.isna(v): return "—"
        # Format Indian numbering with commas (lakh/crore not used; use raw)
        return f"₹{v:,.0f}"

    # === Stats main table ===
    cols_show = ["OM25"] + [c for c in INDICES if c in stats_idx]
    ...
    for label, key, fmt in metric_defs:
        ...
        for col in cols_show:
            src = stats_port if col == "OM25" else stats_idx.get(col, {})
            cells += f"<td>{fmt(src.get(key))}</td>"
        stats_rows += f"<tr><th>{label}</th>{cells}</tr>"

    # === Vs each index (beta etc) ===
    vs_header = ""
    vs_rows = ""
    for col in INDICES:
        v = vs_idx.get(col, {})
...
<head>
<meta charset="UTF-8">
<title>OM25 Production Backtest Report</title>
<style>
  * {{ box-sizing: border-box; }}
  ...
  h2 {{ font-size: 20px; color: #1976d2; margin: 30px 0 15px; padding-bottom: 8px; border-bottom: 2px solid #e0e0e0; }}
  ...
  .summary-cell {{ padding: 16px; background: #fafafa; border-radius: 6px; border-left: 3px solid #1976d2; }}
  ...
  .config-list strong {{ color: #1976d2; }}
...
<body>
<div class="container">
  <h1>OM25 Production Backtest</h1>
  <div class="meta">
    <span>Period: {period_str}</span>
    <span>{stats_port['years']:.2f} years</span>
    <span>Universe: NSE Nifty 250</span>
    <span>Cadence: Bi-weekly</span>
  </div>
  ...
  <div class="card">
    <h2>OM25 Configuration</h2>
    <ul class="config-list">
      <li><strong>Universe:</strong> NSE Nifty 250 ({len(cols)} symbols)</li>
      <li><strong>Cadence:</strong> Bi-weekly entry, weekly exit checks</li>
      <li><strong>Score (bull):</strong> 0.5 × pct_rank(UC) + 0.5 × pct_rank(CR)</li>
      <li><strong>Score (bear):</strong> pct_rank(CR) only</li>
      <li><strong>Regime signal:</strong> NIFTY 100 close vs 100-DMA, 3-day confirmation</li>
      <li><strong>Lookback:</strong> 252 trading days (≥220 obs required)</li>
      <li><strong>Top-N:</strong> 25 stocks · Exit buffer: 20 (drop below rank 45)</li>
      <li><strong>Return filter:</strong> ON (positive 252d return required)</li>
      <li><strong>Drawdown stop:</strong> 20% from peak (weekly check)</li>
  ...
  <div class="card">
    ...
    <p class="note">Each line indexed to 100 at its first valid date in the OM25 period.
  ...
  <div class="card">
    <h2>OM25 vs Each Index — Beta, Alpha, Capture</h2>
    <table>
      <thead><tr>
        <th>Index</th>
        <th>Beta</th>
    ...
    <p class="note">All figures use daily returns over the overlapping period. Up/Down-capture > 1 means OM25 captures more than 100% of index up/down day returns.</p>
  </div>
  ...
  <div class="card">
    <h2>OM25 Monthly Returns Heatmap</h2>
  ...
  <div class="footer">Generated {pd.Timestamp.now().strftime('%Y-%m-%d %H:%M')}  ·  OM25 production locked-in config</div>
</div>
</body>
</html>"""
    ...
    out_path = out_dir / f"om25_production_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}.html"
    out_path.write_text(html)

    artif = ROOT / "tasks/oos_retune_2026/winner_artifacts"
    eq.to_csv(artif / "om25_production_2016plus_equity.csv", index=False)
    trades.to_csv(artif / "om25_production_2016plus_trades.csv", index=False)
    exits.to_csv(artif / "om25_production_2016plus_exits.csv", index=False)
    holdings_df.to_csv(artif / "om25_production_current_holdings.csv", index=False)
