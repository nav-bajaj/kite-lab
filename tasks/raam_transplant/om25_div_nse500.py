"""OM25-DIV (NSE 500) — E1 selection nudge on Quality Momentum, but the
OM25 score selects from the NSE 500 universe instead of Nifty 250.
Everything else is identical to om25_div.py (score, engine, penalty, λ tuning).
...
from e1_l6div import make_l6div_score, holdings_overlap, load_index_close, NIFTY100_INDEX, NSE500_UNIVERSE_CSV  # noqa: E402
...
def main():
    ...
    out_dir = ROOT / "tasks/raam_transplant/runs" / f"om25_div_nse500_{ts}"
    ...
    nse500 = load_universe(ROOT / NSE500_UNIVERSE_CSV)  # <-- only change: NSE 500 universe
    ...
    print("\n" + "=" * 78)
    print(f"OM25-DIV (NSE 500) — selection de-crowding on Quality Momentum, NSE 500 universe (λ*={lam_star})")
    print("=" * 78)
