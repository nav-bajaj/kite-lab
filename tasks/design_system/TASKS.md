...
| 5.4 | **Done 2026-06-05.** `/insights/sectors` (RS leaderboard → SectorBars + branded detail table; constituent-breadth cards; subgroup tracker — `byParent` via `Map` to avoid the object-injection sink) + `/insights/watchlists` (branded list sections, `Pct` for day moves, brand-token validity badges) + `RegimeLegend` (brand cards, tone-coloured dots) + `SnapshotPicker` (brand presets/input) — every hard-coded neutral/emerald/amber/red swapped for brand tokens + semantic `--positive`/`--negative`. `next build` green. | ☑ |
