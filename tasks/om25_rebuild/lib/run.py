...
DEFAULTS = dict(universe="nifty250", score="5050", regimes=1, roc_n=31, confirm=3,
...
def run_candidate(**overrides):
    ...
    if cfg["regimes"] == 2:
        reg = (roc_regime(REGIME_INDEX, cfg["roc_n"], cfg["confirm"], cal) if cfg["regime_kind"] == "roc"
    ...
    res = run_strategy(close_panel=close, trade_panel=trade, calendar=cal, benchmark_aligned=p["bench"],
                       ...
                       regime_panel=None, bear_exposure=0.0, membership_fn=membership_fn, initial_capital=1_000_000)
