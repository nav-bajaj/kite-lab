...
Variants test (persist, dd, mom) weights:
  Equal:        1/3, 1/3, 1/3
  Persist heavy: 50/25/25
  DD heavy:     25/50/25
  Mom heavy:    25/25/50
  Persist+DD:   50/50/0
  Persist+Mom:  50/0/50
  DD+Mom:       0/50/50
  Just persist: 100/0/0
  Just DD:      0/100/0
  Just mom:     0/0/100
"""
import sys
...
def make_score_fn(w_persist, w_dd, w_mom):
    def score(signal_date,
        ...
        weighted = (w_persist * persistence.fillna(0)
...
variants = [
    ...
    ('Persist heavy 50/25/25', 0.50, 0.25, 0.25),
    ('DD heavy 25/50/25',  0.25, 0.50, 0.25),
    ('Mom heavy 25/25/50', 0.25, 0.25, 0.50),
    ('Persist+DD 50/50/0', 0.50, 0.50, 0.00),
    ('Persist+Mom 50/0/50',0.50, 0.00, 0.50),
    ('DD+Mom 0/50/50',     0.00, 0.50, 0.50),
    ('Persist only',       1.00, 0.00, 0.00),
    ('DD only',            0.00, 1.00, 0.00),
    ('Momentum only',      0.00, 0.00, 1.00),
]
...
for u_name, u_path in universes:
    for v_name, wp, wd, wm in variants:
        fn = make_score_fn(wp, wd, wm)
        m = run(universe_path=u_path, score_fn=fn,
