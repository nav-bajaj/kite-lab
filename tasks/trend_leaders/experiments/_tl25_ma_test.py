"""TL25 — test MA Structure component variants.

Locked-in changes from previous reviews:
...
- Current: 4 sub-scores × 0.25 binary
- Drop slope: 3 sub-scores × 0.333 (drop the 200 DMA slope sub-score
  since it's already in eligibility)
- Stacked only: 1 binary (Close > 50 > 100 > 200)
- Distance from 50 DMA pct-ranked: continuous granularity
- Drop MA structure (3 components × 33.3%)
"""
import sys
...
def make_score_fn(*, ma_variant='current', drop_ma=False):
    """Build TL25 scoring function with custom MA structure component.

    ma_variant:
      'current'    - 4 sub-scores × 0.25 (binary)
      'no_slope'   - 3 sub-scores × 0.333 (drop slope; binary)
      'stacked_only' - 1 binary: Close > 50 > 100 > 200
      'distance_50'  - (Close - 50 DMA) / 50 DMA, pct-ranked among eligibles
    """
    def score(signal_date,
        ...
        s200_20ago = (s200_20ago_panel.loc[signal_date]
        ...
        elif ma_variant == 'no_slope':
            ma = pd.Series(0.0, index=close_t.index)
            ma += (close_t > s50_t).astype(float) * (1/3)
            ma += (s50_t > s100_t).astype(float) * (1/3)
            ma += (s100_t > s200_t).astype(float) * (1/3)
        elif ma_variant == 'stacked_only':
            stacked = ((close_t > s50_t) & (s50_t > s100_t)
                       & (s100_t > s200_t))
            ma = stacked.astype(float)
        elif ma_variant == 'distance_50':
            dist = (close_t - s50_t) / s50_t
            dist_eligible = dist.where(eligible).dropna()
            if len(dist_eligible) > 1:
                ranked = dist_eligible.rank(method='average', ascending=True)
                pct = (ranked - 1) / (len(ranked) - 1)
                ma = pct.reindex(close_t.index).fillna(0)
            else:
                ma = pd.Series(0.5, index=close_t.index)

        # Persistence: 252-day window, 100 DMA — locked-in
        ...
        # Drawdown: concave (squared), 126d — locked-in
        ...
        # Momentum: 63d / pct rank — locked-in
        mom_window = 63
        ...
        # Weights
        if drop_ma:
            w_ma, w_persist, w_dd, w_mom = (0, 1/3, 1/3, 1/3)
        else:
            w_ma, w_persist, w_dd, w_mom = (0.25, 0.25, 0.25, 0.25)

        weighted = (w_ma * ma.fillna(0)
                    + w_persist * persistence.fillna(0)
                    + w_dd * dd_score.fillna(0)
                    + w_mom * mom_score.fillna(0))
...
variants = [
    ('Current (4 sub × 0.25)',  make_score_fn(ma_variant='current')),
    ('Drop slope (3 sub × 0.33)', make_score_fn(ma_variant='no_slope')),
    ('Stacked only (1 binary)',   make_score_fn(ma_variant='stacked_only')),
    ('Distance from 50 DMA',       make_score_fn(ma_variant='distance_50')),
    ('Drop MA structure',          make_score_fn(drop_ma=True)),
]
...
print(f"\n{'Universe':<12} {'Variant':<28} {'CAGR':>7} {'Max DD':>8} {'LongDD':>7} "
