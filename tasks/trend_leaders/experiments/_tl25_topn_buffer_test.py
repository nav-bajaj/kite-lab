"""TL25 — Top-N × Exit buffer grid.

All other parameters locked-in:
- Eligibility: Close > 200, 50 > 200, 200 rising 20d
- 5x ATR no floor exit
- Concave squared drawdown 126d
- Persistence 252d / 100 DMA
- Momentum 63d / pct rank
- No MA structure (1/3 each: persist, dd, mom)
- Bi-weekly entry, weekly exit

Grid: Top-N {20, 25, 30} × Buffer {15, 20, 25} = 9 cells per universe
"""
import sys
...
def run(*, universe_path, top_n, buffer, label,
        max_w_override=None):
    ...
    # Scale max_weight inversely with top_n so it's not the binding constraint
    # Default: 7.5% works for top_n=25 (1/25=4% target, 7.5% cap is loose).
    # For top_n=20, 1/20=5% so 7.5% cap still loose. For top_n=30, 1/30=3.3%.
    if max_w_override is None:
        max_w = max(0.075, 1.5 / top_n)
    else:
        max_w = max_w_override
    ...
    res = run_strategy(
        ...
        top_n=top_n, exit_buffer=buffer,
        atr_mult=5.0, atr_min_floor=0.0,
        max_weight=max_w, slippage=0.002, use_trailing_stop=True
    )
    if res is None:
        return None
...
print(f"\n{'Universe':<10} {'Top-N':>5} {'Buffer':>6}  "
      f"{'CAGR':>7} {'Max DD':>8} {'LongDD':>7} "
...
for u_name, u_path in universes:
    for tn in top_ns:
        for buf in buffers:
            label = f'{u_name} N={tn} B={buf}'
            m = run(universe_path=u_path, top_n=tn, buffer=buf, label=label)
            if m:
                print(f"{u_name:<10} {tn:>5} {buf:>6}  "
