"""Day-by-day review of the last 252 trading days: the two new books, the two legacy production books, benchmarks. HTML -> PDF."""
...
cal = mm_eq.index; days = cal[-252:]; a, z = days[0], days[-1]
...
ax.set_title(f"Last 252 trading days, {a.date()} → {z.date()}: cumulative return, % (shaded = ROC31 regime bear)"); ax.legend(frameon=False, ncol=4, fontsize=7); img1 = png(fig)
...
summ = "<table><thead><tr><th>Series</th><th>252-day return</th><th>Max DD in window</th><th>Worst day</th><th>Best day</th><th>Up days</th><th>Longest losing streak</th><th>Days below running high</th><th>Ann. vol</th><th>Order days</th><th>Orders</th><th>Data to</th></tr></thead><tbody>"
for k, s in ST.items():
...
html = f"""<!doctype html><html><head><meta charset='utf-8'><title>Last 252 trading days — day by day</title>{css}</head><body>
<h1>Last 252 trading days, day by day</h1><div class='sub'>{a.date()} → {z.date()} · new books from the honest store (net 20 bps); production books from their own pipeline files (to 2026-08-21, own price panel); benchmarks are price indices; MidSmall 400 real index to 2026-05 then spliced · generated 2026-09-11</div>
...
(OUT / "review252.html").write_text(html)
subprocess.run(["/Applications/Google Chrome.app/Contents/MacOS/Google Chrome", "--headless=new", "--disable-gpu", "--no-pdf-header-footer", f"--print-to-pdf={OUT/'review252.pdf'}", str(OUT / "review252.html")], capture_output=True)
