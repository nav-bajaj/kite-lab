...
DEFAULTS = dict(universe="nifty250", kind="abs", lookback=126, min_obs=110, skip=0, vol_floor=0.05, positive_only=False,
                ...
                min_hold_days=0, fill_from_buffer=False, trim_to_target=0.0, stop_check="weekly", rebalance_day=1, sector_cap=0, satellite_slots=0, universe_cap=0, turnover_floor=0.0, sizing="equal", iv_window=63, dyn_n_bear=0, dyn_mode="lever", bear_buffer=-1, vol_target=0.0, vol_window=21, str_kind="breadth_ma", str_len=200, str_thresh=0.3, str_mode="abs", mix_w=0.5, cr_quantile=0.0, vol_kick="none", vol_k=0.0, regimes=1, bull_kind="abs", overlay=False, regime_kind="roc", roc_n=31, confirm=3, bear_exposure=1.0, reenter_on_flip=False, stop_reentry_block=0, start="2010-01-01", end="2015-12-31")
_ID_OPTIONAL = {"mix_w", "trim_to_target", "fill_from_buffer", "stop_check", "rebalance_day", "sector_cap", "satellite_slots", "bear_buffer", "universe_cap", "turnover_floor", "sizing", "iv_window", "dyn_n_bear", "dyn_mode", "vol_target", "vol_window", "str_kind", "str_len", "str_thresh", "str_mode", "min_hold_days", "vol_kick", "vol_k", "cr_quantile", "regimes", "bull_kind", "overlay", "regime_kind", "roc_n", "confirm", "bear_exposure", "reenter_on_flip", "stop_reentry_block"}
...
def run_candidate(**overrides):
    ...
    res = _run(close_panel=close.loc[cal_run], trade_panel=trade.loc[cal_run], calendar=cal_run, benchmark_aligned=p["bench"].loc[cal_run],
                          ...
                          membership_fn=membership_fn, min_hold_days=cfg["min_hold_days"], size_weights=size_weights, top_n_fn=top_n_fn, sector_of=sector_of, sector_cap=cfg["sector_cap"] or None, fill_from_buffer=cfg["fill_from_buffer"], trim_to_target=cfg["trim_to_target"], initial_capital=1_000_000, stop_reentry_block=cfg["stop_reentry_block"])
