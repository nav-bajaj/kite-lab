...
def _divergence_section(conn, day, spotinfo, out):
    """Spot index vs chain parity-forward, per minute. Under the new 15:40
    timings the official close printed +150/+200 above the late tape on both
    days it first appeared. We measure the dislocation against the day's OWN
    median spot-forward gap so cost-of-carry (a near-constant intraday offset)
    is absorbed and only the anomaly remains."""
    from app.microstructure.gamma_profile import DIVERGENCE_FLAG_PTS

    fwd = _q(conn, """
        select g.minute, avg(g.underlying) as forward
        from option_greeks_minute g
        where date(g.minute)=:d and g.underlying is not null
          and g.expiry=(select min(expiry) from option_greeks_minute where date(minute)=:d)
        group by g.minute
    """, d=day)
    spot = _q(conn, """
        select minute, close as spot from option_minute_bars
        where contract_id='NIFTY_SPOT' and date(minute)=:d
    """, d=day)
    out.append("## Spot vs derivatives divergence (parity forward)")
    if fwd.empty or spot.empty:
        out.append("- insufficient data (no greeks/spot bars)")
        return None
    m = spot.merge(fwd, on="minute", how="inner")
    if m.empty:
        out.append("- no overlapping spot/forward minutes")
        return None
    m["hm"] = _ist(m.minute).dt.strftime("%H:%M")
    m["gap"] = m.spot - m.forward            # ~ -carry in normal conditions
    base = float(m.gap.median())             # the day's baseline gap
    m["dev"] = m.gap - base                   # dislocation vs the day's own baseline
    peak = m.loc[m.dev.abs().idxmax()]
    close_row = m.sort_values("minute").iloc[-1]
    out.append(f"- Baseline spot-forward gap {base:+.1f} pts (carry; spot below forward is normal)")
    out.append(f"- Intraday max dislocation {peak.dev:+.1f} pts at {peak.hm} "
               f"(spot {peak.spot:.1f} vs forward {peak.forward:.1f})")
    out.append(f"- Close window ({close_row.hm}): spot {close_row.spot:.1f} vs "
               f"forward {close_row.forward:.1f} = {close_row.dev:+.1f} pts vs baseline")
    if abs(peak.dev) > DIVERGENCE_FLAG_PTS:
        out.append(f"- **DIVERGENCE FLAG**: {peak.dev:+.1f} pts exceeds {DIVERGENCE_FLAG_PTS:.0f} — "
                   f"official/derivatives dislocation; anything marked or settled off the "
                   f"15:20-15:30 tape inherits this. Surface, do not discard.")
    else:
        out.append(f"- No dislocation beyond the {DIVERGENCE_FLAG_PTS:.0f}-pt flag band.")
    return {"baseline_gap": base, "max_dev": float(peak.dev), "max_dev_hm": peak.hm,
            "close_dev": float(close_row.dev)}
...
def generate(day: str, database_url: Optional[str] = None) -> str:
    ...
    with engine.connect() as conn:
        ...
        if spotinfo:
            ...
            _divergence_section(conn, day, spotinfo, out)
            ...
            try:
                from app.microstructure.day_plan import build_plan, render_plan

                out.extend(render_plan(build_plan(conn, day)))
            except Exception as exc:
                out.append(f"## Morning day-plan (ADVISORY)\n- failed: {exc}")
    engine.dispose()
    out.append("")
