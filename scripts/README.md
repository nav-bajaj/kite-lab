...
| **Portfolio runners** | `run_final_momentum_portfolio.py`, `run_om25_v3_portfolio.py`, `run_tl25_v3_portfolio.py`, `run_l6_v2_portfolio.py`, `run_combo_defensive_portfolio.py`, `run_rebuilt_book.py` + `rebuilt_books.py`, `update_all_portfolios.py` | The 4 production portfolios + the legacy L6 helper, the two rebuilt books (`mm_v1`, `om25_v4`, read the master store — see `tasks/mm_rebuild/MECHANICS.md`), plus the orchestrator that runs all of them |
