"""Asymmetric regime trigger sweep — slow bear, fast bull.

Bear entry stays: NIFTY 100 < 100-DMA for 3 consecutive days.

Bull return varies:
  A. fast_ma_50_1   — price > 50-DMA + 1-day confirm
  B. fast_ma_21_1   — price > 21-DMA + 1-day confirm (more aggressive)
  C. stddev_snap_0.5 — price recovers to within 0.5σ of 100-DMA after being
                       below -1σ (Bollinger-style snapback, user's suggestion)
  D. stddev_snap_0.25 — tighter snapback
  E. recent_low_5pct — price > 105% × trailing 20-day low (pure
                       mean-reversion bounce)
  F. symmetric_100_3 — current symmetric trigger (reference)

All tested on COMBO 50/50 Friday biweekly + ALT 1 + bear=30%.
Reports trailing returns (1mo/3mo/6mo/YTD), aggregate OOS metrics,
walk-forward pass rate, and recent regime flip log.
"""
from __future__ import annotations

import math
import sys
from pathlib import Path

import pandas as pd

ROOT = Path(__file__).resolve().parents[1]
if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))

from scripts._clean_engine import run_strategy, biweekly_fridays, fridays
from scripts._momentum_engine import (
    BASELINE as MM_BASELINE, build_momentum_panels, make_momentum_score,
    lookback_months_to_days,
)
from scripts.om25_v3 import (
    LOCKED as OM25_LOCKED, build_regime_panel_confirmed, make_om25_tilt_score,
)
from scripts.combo_defensive import LOCKED as COMBO_LOCKED, make_combo_score_fn
from scripts.backtest_momentum import load_price_panels, load_benchmark
from scripts.build_om25_signals import load_universe
from scripts.multi_window_oos_eval import period_metrics


END_DATE = "2026-05-12"


def build_asymmetric_regime(
    idx_path, calendar, *,
    bear_ma=100, bear_confirm=3,
    bull_trigger="fast_ma",
    bull_fast_ma=50, bull_fast_confirm=1,
    bull_stddev_main_ma=100, bull_stddev_window=252,
    bull_stddev_oversold=1.0,    # entered "oversold" when price < MA - this many σ
    bull_stddev_recovery=0.5,    # exit bear when price > MA - this many σ
    bull_low_window=20, bull_low_pct=0.05,
):
    """Build a bool regime panel with asymmetric bear-entry vs bull-return.

    Bear entry: price < bear_ma for bear_confirm consecutive days.
    Bull return: depends on bull_trigger choice.
    """
    idx = pd.read_csv(idx_path, parse_dates=["date"])
    idx["date"] = pd.to_datetime(idx["date"]).dt.tz_localize(None).dt.normalize()
    idx = idx.sort_values("date").set_index("date")["close"].astype(float)

    sma_main = idx.rolling(bear_ma, min_periods=bear_ma).mean()
    below_main = idx < sma_main

    # Precompute bull-trigger signals
    if bull_trigger == "fast_ma":
        sma_fast = idx.rolling(bull_fast_ma, min_periods=bull_fast_ma).mean()
        above_fast = idx > sma_fast
    elif bull_trigger.startswith("stddev_snap"):
        sma_main_for_stddev = idx.rolling(bull_stddev_main_ma).mean()
        pct_dev = (idx - sma_main_for_stddev) / sma_main_for_stddev
        rolling_std = pct_dev.rolling(bull_stddev_window).std()
        oversold_threshold = -bull_stddev_oversold * rolling_std
        recovery_threshold = -bull_stddev_recovery * rolling_std
    elif bull_trigger == "recent_low_bounce":
        rolling_low = idx.rolling(bull_low_window, min_periods=1).min()
        above_low = idx > rolling_low * (1 + bull_low_pct)

    # State machine
    state = []
    current = True   # start bull
    bear_consec = 0  # consecutive days price < bear_ma (for bear-entry confirm)
    bull_confirm_count = 0  # consecutive days bull-return signal active
    has_been_oversold = False  # tracks "got stretched" for stddev_snap

    for d in idx.index:
        if current:
            # In bull regime — check for bear entry
            below = below_main.get(d, False)
            if below and not pd.isna(below):
                bear_consec += 1
                if bear_consec >= bear_confirm:
                    current = False
                    bear_consec = 0
                    bull_confirm_count = 0
                    has_been_oversold = False
            else:
                bear_consec = 0
        else:
            # In bear regime — check for bull return
            bear_consec = 0
            triggered = False
            if bull_trigger == "fast_ma":
                signal = above_fast.get(d, False)
                if signal and not pd.isna(signal):
                    bull_confirm_count += 1
                    if bull_confirm_count >= bull_fast_confirm:
                        triggered = True
                else:
                    bull_confirm_count = 0
            elif bull_trigger.startswith("stddev_snap"):
                dev = pct_dev.get(d)
                ot = oversold_threshold.get(d)
                rt = recovery_threshold.get(d)
                if dev is None or pd.isna(dev) or pd.isna(ot) or pd.isna(rt):
                    pass
                else:
                    if dev < ot:
                        has_been_oversold = True
                    if has_been_oversold and dev > rt:
                        triggered = True
            elif bull_trigger == "recent_low_bounce":
                signal = above_low.get(d, False)
                if signal and not pd.isna(signal):
                    triggered = True
            if triggered:
                current = True
                bull_confirm_count = 0
                has_been_oversold = False
        state.append(current)

    out = pd.Series(index=idx.index, data=state, dtype=bool)
    out = out.shift(1).fillna(True)  # lag 1 day, default bull pre-data
    if calendar is not None:
        out = out.reindex(calendar).ffill().fillna(True).astype(bool)
    return out


def make_combo(score_fns, n_per=12):
    def score_fn(signal_date, **_):
        picked = set(); rows = []
        for _, sf in score_fns:
            scores = sf(signal_date)
            if scores is None or scores.empty: continue
            ranked = scores.dropna().sort_values(ascending=False)
            taken = 0
            for sym in ranked.index:
                if sym in picked: continue
                picked.add(sym); rows.append(sym); taken += 1
                if taken >= n_per: break
        if not rows: return pd.Series(dtype=float)
        n = len(rows)
        return pd.Series({sym: float(n - i) for i, sym in enumerate(rows)})
    return score_fn


def _calmar(c, d):
    if d is None or pd.isna(d) or abs(d) < 1e-6: return None
    return c / abs(d)


def slice_metrics(eq, start, end):
    m = period_metrics(eq, "x", start, end)
    cagr = m.get("cagr_pct"); dd = m.get("max_dd_pct"); sh = m.get("sharpe")
    return {
        "cagr_pct": round(cagr, 2) if cagr is not None else None,
        "sharpe": round(sh, 2) if sh is not None else None,
        "calmar": round(_calmar(cagr, dd), 2) if _calmar(cagr, dd) is not None else None,
        "max_dd_pct": round(dd, 2) if dd is not None else None,
    }


def trailing(eq, end_date):
    pv = eq.set_index("date")["pv"].astype(float)
    pv.index = pd.to_datetime(pv.index)
    pv = pv[pv.index <= pd.Timestamp(end_date)]
    if pv.empty: return {}
    out = {}
    for days, label in [(21, "1mo"), (63, "3mo"), (126, "6mo")]:
        if len(pv) > days:
            out[label] = round((pv.iloc[-1] / pv.iloc[-days - 1] - 1) * 100, 2)
    ytd = pv[pv.index >= pd.Timestamp("2026-01-01")]
    if not ytd.empty:
        out["YTD_2026"] = round((ytd.iloc[-1] / ytd.iloc[0] - 1) * 100, 2)
    return out


def regime_flips_recent(panel, end_date, days_back=180):
    end_ts = pd.Timestamp(end_date)
    start = end_ts - pd.Timedelta(days=days_back)
    sub = panel[(panel.index >= start) & (panel.index <= end_ts)]
    flips = []
    prev = None
    for d, v in sub.items():
        if prev is not None and v != prev:
            flips.append((d.date(), "BULL" if v else "BEAR"))
        prev = v
    return flips


WF_WINDOWS = {
    f"W{i:02d}": (f"{2009 + i}-09-01", f"{2009 + i + 3}-08-31",
                   f"{2009 + i + 3}-09-01", f"{2009 + i + 4}-08-31")
    for i in range(1, 14)
}
WF_WINDOWS["W13"] = ("2022-09-01", "2025-08-31", "2025-09-01", "2026-05-08")


def main():
    print("[load] panels ...")
    close_panel, trade_panel = load_price_panels(ROOT / "nse500_data_merged")
    calendar = close_panel.index
    benchmark = load_benchmark(ROOT / "data/benchmarks/nifty100.csv")
    benchmark_aligned = benchmark.reindex(calendar).ffill()
    sma_200 = close_panel.rolling(200, min_periods=200).mean()
    atr_20 = close_panel.pct_change().rolling(20).std()

    nse500_uni = load_universe(ROOT / "data/static/nse500_universe.csv")
    nse500_cols = [s for s in close_panel.columns if s in nse500_uni]
    l6_panels = build_momentum_panels(
        close_panel[nse500_cols],
        lookback_days=lookback_months_to_days(MM_BASELINE["lookback_months"]),
        skip_days=MM_BASELINE["skip_days"],
    )
    l6_score = make_momentum_score(
        l6_panels, vol_floor=MM_BASELINE["vol_floor"],
        vol_power=MM_BASELINE["vol_power"], cross_sectional_zscore=True,
    )
    nifty250_uni = load_universe(ROOT / "data/static/nifty250_universe.csv")
    nifty250_cols = [s for s in close_panel.columns if s in nifty250_uni]
    om25_returns = close_panel[nifty250_cols].pct_change()
    om25_regime_for_score = build_regime_panel_confirmed(
        ROOT / "indices_data_historical/NIFTY_100.csv", 100, 3, calendar=calendar,
    )
    om25_score = make_om25_tilt_score(
        om25_returns, om25_regime_for_score,
        bull_w_uc=OM25_LOCKED["bull_w_uc"], bull_w_cr=OM25_LOCKED["bull_w_cr"],
        bear_w_uc=OM25_LOCKED["bear_w_uc"], bear_w_cr=OM25_LOCKED["bear_w_cr"],
        return_filter=OM25_LOCKED["return_filter"],
        lookback=OM25_LOCKED["lookback"], min_obs=OM25_LOCKED["min_obs"],
    )

    combo = make_combo([("L6", l6_score), ("OM25", om25_score)], n_per=12)
    ctx_kwargs = dict(close_panel=close_panel, trade_panel=trade_panel,
                       calendar=calendar, benchmark_aligned=benchmark_aligned,
                       sma_200_panel=sma_200, atr_20_panel=atr_20)

    # Build the various regime panels
    print("[build] regime panels ...")
    idx_path = ROOT / "indices_data_historical/NIFTY_100.csv"
    regimes = {
        "F. symmetric_100_3 (reference)": build_regime_panel_confirmed(
            idx_path, 100, 3, calendar=calendar),
        "A. fast_ma_50_1 (50-DMA, 1-day conf)": build_asymmetric_regime(
            idx_path, calendar, bull_trigger="fast_ma",
            bull_fast_ma=50, bull_fast_confirm=1),
        "B. fast_ma_21_1 (21-DMA, 1-day conf)": build_asymmetric_regime(
            idx_path, calendar, bull_trigger="fast_ma",
            bull_fast_ma=21, bull_fast_confirm=1),
        "C. stddev_snap_-1σ→-0.5σ": build_asymmetric_regime(
            idx_path, calendar, bull_trigger="stddev_snap",
            bull_stddev_oversold=1.0, bull_stddev_recovery=0.5),
        "D. stddev_snap_-1σ→-0.25σ (tighter)": build_asymmetric_regime(
            idx_path, calendar, bull_trigger="stddev_snap",
            bull_stddev_oversold=1.0, bull_stddev_recovery=0.25),
        "E. recent_low_bounce_5%": build_asymmetric_regime(
            idx_path, calendar, bull_trigger="recent_low_bounce",
            bull_low_window=20, bull_low_pct=0.05),
    }

    # Show recent regime flip patterns per variant
    print(f"\n{'=' * 110}")
    print("REGIME FLIP PATTERN — last 6 months per variant")
    print(f"{'=' * 110}")
    for label, panel in regimes.items():
        flips = regime_flips_recent(panel, END_DATE, days_back=180)
        end_state = "BULL" if panel.iloc[-1] else "BEAR"
        print(f"\n[{label}] now: {end_state}")
        for d, s in flips:
            print(f"  {d}: → {s}")

    # Run each variant
    print(f"\n[run] backtests ...")
    equities = {}
    for label, regime in regimes.items():
        entry_all = biweekly_fridays(calendar)
        weekly_all = fridays(calendar)
        s_ts = pd.Timestamp("2009-09-01"); e_ts = pd.Timestamp(END_DATE)
        entry_dates = entry_all[(entry_all >= s_ts) & (entry_all <= e_ts)]
        weekly_filt = weekly_all[(weekly_all >= s_ts) & (weekly_all <= e_ts)]
        res = run_strategy(
            **ctx_kwargs,
            entry_signal_dates=entry_dates, weekly_signal_dates=weekly_filt,
            signal_function=combo, signal_function_args={},
            top_n=24, exit_buffer=0,
            max_weight=0.075, slippage=0.002,
            atr_mult=0.0, atr_min_floor=0.0,
            use_trailing_stop=False, use_dma_exit=False,
            weekly_rank_check=False,
            regime_panel=regime, bear_exposure=0.30,
            bear_skips_entries=False,    # ALT 1
            min_hold_days=8, initial_capital=1_000_000,
        )
        equities[label] = res["equity"]
        print(f"  {label}", flush=True)

    # === Trailing returns ===
    print(f"\n{'=' * 110}")
    print("TRAILING RETURNS through 2026-05-12 (the key 'are we capturing the rally?' metric)")
    print(f"{'=' * 110}")
    rows = []
    for label, eq in equities.items():
        rows.append({"variant": label, **trailing(eq, END_DATE)})
    print(pd.DataFrame(rows).to_string(index=False))

    # === Aggregate metrics ===
    print(f"\n{'=' * 110}")
    print("AGGREGATE METRICS")
    print(f"{'=' * 110}")
    agg_rows = []
    for label, eq in equities.items():
        for w_id, s, e in [("OOS_full", "2017-01-01", "2026-05-08"),
                            ("Prod window", "2020-07-10", "2026-05-08")]:
            agg_rows.append({"variant": label, "window": w_id,
                              **slice_metrics(eq, s, e)})
    agg = pd.DataFrame(agg_rows)
    for w in ["OOS_full", "Prod window"]:
        sub = agg[agg["window"] == w]
        if sub.empty: continue
        print(f"\n--- {w} ---")
        cols = ["variant", "cagr_pct", "sharpe", "calmar", "max_dd_pct"]
        print(sub[cols].to_string(index=False))

    # === Walk-forward pass rates ===
    print(f"\n{'=' * 110}")
    print("WALK-FORWARD (13 windows)")
    print(f"{'=' * 110}")
    wf_rows = []
    for w_id, (_, _, oos_s, oos_e) in WF_WINDOWS.items():
        for label, eq in equities.items():
            m = slice_metrics(eq, oos_s, oos_e)
            wf_rows.append({"window": w_id, "variant": label,
                              "oos_sharpe": m["sharpe"],
                              "oos_pass": (m["sharpe"] is not None and m["sharpe"] >= 0.7)})
    wf_df = pd.DataFrame(wf_rows)
    summary = (wf_df.groupby("variant")
                  .agg(n_pass=("oos_pass", "sum"),
                       mean_oos_sharpe=("oos_sharpe", "mean"),
                       median_oos_sharpe=("oos_sharpe", "median"))
                  .round(2))
    summary["pass_rate_pct"] = (summary["n_pass"] / 13 * 100).round(0)
    print(summary.to_string())

    pd.DataFrame(rows).to_csv(ROOT / "tasks/MM-tuning/asymmetric_regime_trailing.csv", index=False)
    agg.to_csv(ROOT / "tasks/MM-tuning/asymmetric_regime_agg.csv", index=False)
    wf_df.to_csv(ROOT / "tasks/MM-tuning/asymmetric_regime_wf.csv", index=False)


if __name__ == "__main__":
    main()
