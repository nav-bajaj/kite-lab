"""
Run Nifty 250 momentum portfolio with same parameters as final portfolio

This script generates momentum signals and backtests for the Nifty 250 universe.
Uses identical parameters to the NSE 500 final portfolio for direct comparison.

Usage:
    python scripts/run_nifty250_momentum.py
    python scripts/run_nifty250_momentum.py --dry-run
"""
...
def main():
    parser = argparse.ArgumentParser(description="Nifty 250 momentum portfolio generator")
    ...
    parser.add_argument("--universe-file", type=Path, default=Path("data/static/nifty250_universe.csv"))
    parser.add_argument("--output-root", type=Path, default=Path("nifty_250_tests"))
    ...
    output_dir = args.output_root / f"nifty250_portfolio_{timestamp}"
    ...
    signals_path = signals_dir / "nifty250_signals.csv"

    print(f"\n{'='*80}")
    print(f"NIFTY 250 MOMENTUM PORTFOLIO")
    print(f"{'='*80}\n")
    print(f"Universe: Nifty 250 (250 stocks)")
