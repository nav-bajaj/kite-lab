...
def main():
    parser = argparse.ArgumentParser(description="Monte Carlo for L3 momentum (baseline, hysteresis, PnL-hold)")
    ...
    parser.add_argument("--vol-power", nargs="+", type=float, default=[1.0])
    ...
    exp_dir = args.output_root / f"l3_mc_{timestamp}"
    ...
    # Single universe sample reused for all runs
    ...
    for i in range(1, args.runs + 1):
        ...
        vol_power = random.choice(args.vol_power)
        ...
        for scenario_label, scenario_exit_buf, pnl_threshold in scenarios:
            ...
            if scenario_label == "hyst":
                cmd[cmd.index("--scenario") + 1] = "baseline"  # hysteresis via exit buffer only
            elif scenario_label == "pnl_hold":
                cmd[cmd.index("--scenario") + 1] = "baseline"
