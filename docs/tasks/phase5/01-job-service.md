...
Create a service to execute scripts via subprocess with status tracking, log capture, and job management.

## Tasks

- [ ] Create `job_service.py` in `kite-api/app/services/`
- [ ] Implement `create_job()` function
- [ ] Implement `run_job()` background task
- [ ] Implement `get_job()` function
- [ ] Implement `list_jobs()` function
- [ ] Implement `cancel_job()` function
- [ ] Implement `get_log_path()` helper
- [ ] Configure log directory at `logs/jobs/`

## Implementation

### File: `kite-api/app/services/job_service.py`
...
|---------|--------|-------------|
| `daily_pipeline` | `run_daily_pipeline.py` | Full daily pipeline with data fetch |
| `generate_portfolio` | `run_final_momentum_portfolio.py` | Generate portfolio signals |
| `backup_data` | `sync_data_backup.py` | Backup data to external location |
| `fetch_prices` | `fetch_nse500_history.py` | Fetch NSE 500 price data |
| `build_signals` | `build_momentum_signals_flexible.py` | Build momentum signals |
...
cancel_job() → status: "cancelled" (from queued/running)
```
...
[2026-02-14T07:00:01] Command: python scripts/run_daily_pipeline.py --universe nse500
------------------------------------------------------------
<script output>
------------------------------------------------------------
[2026-02-14T07:05:32] Job completed
```
...
# Run in background
asyncio.create_task(JobService.run_job(job.id))

# Check status
job = JobService.get_job(job.id)
print(f"Status: {job.status}")

# Read logs
logs = JobService.read_logs(job.id)
print(logs)
```
...
- Logs are persisted to `logs/jobs/{job_id}.log`
- Default timeout is 30 minutes (configurable)
- Cancelled jobs may continue running in subprocess (no PID tracking)
