...
- `python scripts/run_final_momentum_portfolio.py [--min-entry-score 2.5] [--min-exit-score 1.5] [--score-rebalance-mode incremental]`
  ...
  - **Now includes score filtering by default**: entry threshold 2.5, exit threshold 1.5, incremental allocation mode (proven +3.98% CAGR vs baseline, 13% fewer trades).
  ...
  - Override score thresholds with `--min-entry-score` and `--min-exit-score`, or disable filtering by setting them to 0.
