...
- **Auth:** Supabase verifies session JWTs via JWKS (ES256, issuer-pinned, `aud` required). Role in the server-controlled `app_metadata.role` — `admin`, `preview`, or `client`; `user_metadata` is never consulted. 24 backend mutation/engine endpoints behind `require_admin`; 21 client-read endpoints behind `check_universe_access`. **`preview` passes the site gate and nothing else** (`GATE_ROLES` in `kite-api/app/auth.py`, `canPassGate()` in `kite-dashboard/src/lib/roles.ts`) — never widen it to `require_admin` or `check_universe_access`. `/insights` visibility reads a third, deliberately separate predicate, `canSeeInsightsSandbox()`: "nothing else" has a launch-window asterisk, and the other half of that control is revoking every grant before `SITE_MODE` comes off. Tests at `kite-api/tests/test_supabase_authz.py`, `test_private_mode.py`, `test_supabase_jwt_spec.py` (641 tests) — don't weaken. Runbook: `docs/security/preview-access.md`.
